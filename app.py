...
with tabs[-1]:
    ...
    st.markdown("Adjust AI investment, automation, and training levels to forecast **productivity gain and cost reduction** using regression models.")

    sim_df = df_filtered.copy()

    # choose automation column safely
    auto_col = "task_automation_rate" if "task_automation_rate" in sim_df.columns else "automation_level"

    features = ["ai_investment_per_employee", auto_col, "ai_training_hours"]
    features = [f for f in features if f in sim_df.columns]
    ...
    y_prod = sim_df[target_prod].fillna(0)
    y_cost = sim_df[target_cost].fillna(0)
    ...
    with col1:
        invest = st.slider("AI Investment per Employee ($)", 0, 200000, 30000, step=5000)

    with col2:
        auto = st.slider("Automation Level (%)", 0, 100, 30)

    with col3:
        train = st.slider("AI Training Hours per Employee", 0, 200, 20)

    input_df = pd.DataFrame({
        "ai_investment_per_employee":[invest],
        auto_col:[auto],
        "ai_training_hours":[train]
    })
    ...
    c1, c2 = st.columns(2)
    c1.metric("Predicted Productivity Gain (%)", f"{pred_prod:.2f}")
    c2.metric("Predicted Cost Reduction (%)", f"{pred_cost:.2f}")

    fig_sim = px.scatter(
        x=[invest],
        y=[pred_prod],
        size=[pred_cost],
        labels={"x":"AI Investment","y":"Predicted Productivity"},
        title="Investment vs Predicted Productivity"
    )
    fig_sim.update_layout(template="plotly_dark")
    st.plotly_chart(fig_sim, width="stretch")

    st.markdown("💡 **Insight:** Increasing AI investment and automation generally increases productivity and reduces operational costs. Organizations can use this simulator to estimate potential returns before deciding whether to implement AI internally or rely on consultants.")
